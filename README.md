...
- `favicon-16.png`, `favicon-32.png`, `favicon-180.png` — fallback do ícone em PNG,
  para navegadores que não suportam favicon em SVG (ex.: Safari mais antigo)
- `privacidade.html` — página simples de Política de Privacidade, linkada no
  rodapé e no formulário de contato
...
- **SEO básico**: Open Graph completo, Twitter Card, `canonical`, JSON-LD
  ...
  todos esses arquivos usam o placeholder `https://lipeprieto.github.io/guilherme-dos-santos-personal/`
...
- **Favicon com fallback em PNG**: alguns navegadores mais antigos não leem
  favicon em SVG; agora há também PNGs em 16, 32 e 180px gerados a partir do
  mesmo desenho original.
- **Política de Privacidade**: nova página (`privacidade.html`), linkada no
  rodapé e no formulário de contato, explicando de forma simples e honesta o
  que o site realmente faz hoje (não armazena os dados do formulário, não usa
  cookies/analytics). Atualize a data e o e-mail de contato nela antes de
  publicar.
...
- O texto "número de pessoas atendidas" e qualquer número de atendimentos: só devem
...
8. Depois de saber a URL final, atualize o placeholder
   `https://lipeprieto.github.io/guilherme-dos-santos-personal/` em `index.html` (canonical, Open
...
## Revisão aplicada em 29/07/2026

- URL oficial: `https://lipeprieto.github.io/guilherme-dos-santos-personal/`
- Título SEO: `Nexus Vital | Exercício individualizado para fibromialgia`
- público definido de forma neutra: qualquer pessoa com fibromialgia
- `SITE_CONFIG` disponibilizado também em `window.SITE_CONFIG`
- nome, Instagram, formação e CREF conectados ao `config.js`
- formação e CREF ocultos enquanto estiverem vazios
- validação do nome no formulário de WhatsApp
- loader exibido apenas na primeira visita no navegador
- canvas pausado quando a primeira seção sai da tela
- canvas e 3D desativados automaticamente em celulares modestos
- textura reduzida no celular
- parallax, inclinação e magnetismo suavizados
- `will-change` ativado somente durante a interação
